    ...
    class ppws_product_categories_settings
    {
        ...
        public function ppws_product_categories_register_settings_init()
        {
            ...
            add_settings_field(
                ...
                'ppws-product-categories-password-settings-section',
                'ppws_product_categories_password_settings_section',
                ['type' => 'checkbox', 'label_for' => 'ppws_product_categories_all_categories_field_checkbox', 'class' => "$ppws_product_categories_enable_password_class ppws-product-categories-enable-password-section ppws-product-categories-list-section", 'description' => 'Selected categories are protected with password.']
            );
        ...
        public function ppws_product_categories_password_settings($args)
        {
            ...
            ?>
                <!-- Numberbox -->
                <input type="number" class="ppws-numberbox" min="1" max="400" name="ppws_product_categories_settings[<?php esc_attr_e($args['label_for']); ?>]" id="<?php esc_attr_e($args['label_for']) ?>" value="<?php esc_attr_e($value) ?>" placeholder="<?php esc_attr_e($args['placeholder']) ?>">
                <p class="ppws-note"><?php esc_attr_e($args['description']) ?></p>
            <?php
            }
        }
        ...
        public function ppws_product_categories_select_user_role_settings($args)
        {
            ...
            ?>
                ...
                <p class="ppws-note ppws-userrole-error ppws-hide-section">
                    ...
                    </svg>You must first enable user roles to select any specific user role.
                </p>
            <?php
            }
        }
                ...
                <div class="ppws_all_user_list">
                    ...
                    foreach ($all_categories as $cat) {
                        if ($cat->category_parent == 0) {
                            ...
                            <div class="ppws_all_user_list_input">
                                <label>
                                    <input type="checkbox" class="ppws-checkbox" name="ppws_product_categories_settings[<?php esc_attr_e($args['label_for']) ?>][]" value="<?php esc_attr_e($category_id); ?>" <?php if (in_array($category_id, $select_value)) {
                                                                                                                                                                                                                        esc_attr_e('checked');
                                                                                                                                                                                                                    } ?>><?php _e($cat->name); ?>
                                </label>
                                <?php
                                $fun_args = $args['label_for'];
                                ppws_get_woocommerce_product_categories($category_id, $fun_args, $select_value);
                                ?>
