    ...
    class ppws_whole_site_settings{
        ...
        public function ppws_password_settings($args)
        {
            ...
            if ($args['type'] == 'checkbox') {
                ...
                <label class="ppws-switch">
                    <input type="checkbox" class="ppws-checkbox <?php if(isset($args['custom_class'])) { esc_attr_e($args['custom_class']); }  ?>" name="ppws_general_settings[<?php esc_attr_e( $args[ 'label_for' ] ) ?>]" id="<?php esc_attr_e( $args[ 'label_for' ] ) ?>" value="on" <?php if($value == "on"){ esc_attr_e('checked'); } ?> <?php if($args['label_for'] == "reverse_protection"){ esc_attr_e('disabled'); } ?>>
            ...
            ?>
                <!-- Numberbox -->
                <input type="number" class="ppws-numberbox" max="400" name="ppws_general_settings[<?php esc_attr_e($args['label_for']); ?>]" id="<?php esc_attr_e($args['label_for']) ?>" value="<?php esc_attr_e($value) ?>" placeholder="<?php esc_attr_e($args['placeholder']) ?>" min=0>
                <p class="ppws-note"><?php esc_attr_e($args['description'],'password-protected-store-for-woocommerce') ?></p>
            <?php
            }
        }
       ...
       public function ppws_logged_in_user_role_settings($args)
       {
                ...
                <div class="ppws_all_user_list">
                    ...
                    foreach ($editable_roles as $role) { 
                        ...
                        <div class="ppws_all_user_list_input">                            
                            <label>
                                <input type="checkbox" class="ppws-checkbox ppws_editable_roles_single" name="ppws_general_settings[<?php esc_attr_e( $args[ 'label_for' ] ) ?>][]" id="<?php esc_attr_e( $args[ 'label_for' ] ) ?>" value="<?php esc_attr_e($role['name']); ?>"  <?php if(in_array($role['name'],$value)){ esc_attr_e('checked'); } ?> ><?php esc_attr_e($role['name']) ?>
                            </label>
                        </div>
                        <?php
                        $counter++;
        ...
        public function ppws_select_user_role_settings($args)
        {
            ...
            if(isset($ppws_whole_site_options[$args['label_for']])) {
                if(is_array($ppws_whole_site_options[$args['label_for']])) {
                    $user_role = $ppws_whole_site_options[$args['label_for']];
                }else{
                    $user_role = array($ppws_whole_site_options[$args['label_for']]);
                }
            }

            if ($args['type'] == 'radio') { ?>
                <label class="ppws-label">
                    <input type="checkbox" class="<?php esc_attr_e('ppws_user_non_logged_in_user'); ?>" id="<?php esc_attr_e($args['label_for']); ?>_non_logged_in_user" name="ppws_general_settings[<?php esc_attr_e($args['label_for']); ?>][]" value="non-logged-in-user" <?php if(in_array('non-logged-in-user', $user_role)) echo 'checked="checked"'; ?>><?php _e('Non Logged In User','password-protected-store-for-woocommerce') ?>
                </label>

                <label class="ppws-label">
                    <input type="checkbox" class="<?php esc_attr_e('ppws_user_logged_in_user'); ?>" id="<?php esc_attr_e($args['label_for']); ?>_logged_in_user" name="ppws_general_settings[<?php esc_attr_e($args['label_for']); ?>][]" value="logged-in-user" <?php if(in_array('logged-in-user', $user_role)) echo 'checked="checked"'; ?>><?php _e('Logged In User','password-protected-store-for-woocommerce') ?>
                </label>
           ...
           if (isset($input['ppws_set_password_field_textbox']) && !empty($input['ppws_set_password_field_textbox'])) {
            ...
            if (isset($input['enable_user_role'])) {
				...
				if (isset($input['ppws_select_user_role_field_radio'])) {
					$new_input['ppws_select_user_role_field_radio'] = array_map( 'sanitize_text_field', $input['ppws_select_user_role_field_radio'] );
				}
