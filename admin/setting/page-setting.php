...
if (!class_exists('ppws_page_settings')) {
    ...
    class ppws_page_settings
    {
        ...
        public function ppws_page_select_user_role_settings($args)
        {
            ...
            if(isset($ppws_page_options[$args['label_for']])) {
                if(is_array($ppws_page_options[$args['label_for']])) {
                    $user_role = $ppws_page_options[$args['label_for']];
                }else{
                    $user_role = array($ppws_page_options[$args['label_for']]);
                }
            }
            if ($args['type'] == 'radio') {
            ?>
                <label class="ppws-label"><input type="checkbox" class="<?php esc_attr_e('ppws_user_non_logged_in_user'); ?>" name="ppws_page_settings[<?php esc_attr_e($args['label_for']); ?>][]" value="non-logged-in-user" <?php if(in_array('non-logged-in-user', $user_role)) echo 'checked="checked"'; ?>><?php _e('Non Logged In User', 'password-protected-store-for-woocommerce') ?></label>

                <label class="ppws-label"> <input type="checkbox" class="<?php esc_attr_e('ppws_user_logged_in_user'); ?>" name="ppws_page_settings[<?php esc_attr_e($args['label_for']); ?>][]" value="logged-in-user" <?php if(in_array('logged-in-user', $user_role)) echo 'checked="checked"'; ?>><?php _e('Logged In User', 'password-protected-store-for-woocommerce') ?></label>
        ...
        public function sanitize_settings($input)
        {
            ...
            if (isset($input['enable_user_role'])) {
                ...
                if (isset($input['ppws_page_select_user_role_field_radio'])) {
                    $new_input['ppws_page_select_user_role_field_radio'] = array_map( 'sanitize_text_field', $input['ppws_page_select_user_role_field_radio'] );
                }
