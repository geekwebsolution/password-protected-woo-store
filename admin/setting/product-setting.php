<?php
if (!class_exists('ppws_product_settings')) {
    ...
    class ppws_product_settings
    {
        ...
        function ppws_product_callback() {
            ...
            <form action="options.php?tab=product-setting" method="post" class="ppws-product-setting-form">
                ...
                echo _e('The "Products" password will be applied to selected products. If the "Products" and "Product Categories" and "General" passwords are enabled, then selected products will be asked for "Products" password. ','password-protected-store-for-woocommerce');
                echo '</p>';
                ?>
                <div class="ppws-section">
                    <?php do_settings_sections('ppws-product-password-settings-section'); ?>
                </div>
                <div class="ppws-section ppws-section-user <?php echo $custom_class.' '.$custom_class2; ?>">
                    <?php do_settings_sections('ppws-product-user-role-section');
                    ?>
                </div>
        ...
        public function ppws_product_register_settings_init()
        {
            register_setting(
                'ppws-settings-options',
                'ppws_product_settings',
                array($this, 'product_sanitize_settings')
            );
            ...
            add_settings_field(
                'ppws_product_enable_password_checkbox',
            ...
            add_settings_field(
                'ppws_product_enable_password_checkbox_for_admin',
            ...
            add_settings_field(
                'ppws_product_set_password_textbox',
            ...
            add_settings_field(
                'ppws_product_set_password_expiry_textbox',
            ...
            add_settings_field(
                'ppws_product_list_of_product_checkbox',
                __('Product List', 'password-protected-store-for-woocommerce'),
            ...
            add_settings_field(
                ...
                'ppws-product-password-settings-section',
                'ppws_product_password_settings_section',
                ['type' => 'checkbox', 'label_for' => 'ppws_hide_products_checkbox_field_checkbox', 'description' => '', 'class' => "$ppws_product_enable_password_class ppws-product-enable-password-section", 'description' => 'Hide products from shop/product loop listing.']
            );
            ...
            add_settings_field(
                'ppws_product_user_select_role_radio',
            ...
            add_settings_field(
                'ppws_product_logged_in_user_checkbox',
        ...
        public function ppws_product_password_settings($args)
        {
            global $ppws_product_options;
           
            $value = isset($ppws_product_options[$args['label_for']]) ? $ppws_product_options[$args['label_for']] : '';
            ...
            ?>
                <input type="text" class="ppws-textbox ppws-pwd-input" name="ppws_product_settings[<?php esc_attr_e($args['label_for']) ?>]" id="<?php esc_attr_e($args['label_for']) ?>" placeholder="<?php esc_attr_e($args['placeholder']) ?>" value="<?php esc_attr_e(ppws_decrypted_password($value)); ?>">
                <p class="ppws-note"><?php esc_attr_e($args['description']) ?></p>
            <?php
            } elseif ($args['type'] == 'number') {
            ?>
                <!-- Numberbox -->
                <input type="number" class="ppws-numberbox" max="400" name="ppws_product_settings[<?php esc_attr_e($args['label_for']); ?>]" id="<?php esc_attr_e($args['label_for']) ?>" value="<?php esc_attr_e($value) ?>" placeholder="<?php esc_attr_e($args['placeholder']) ?>">
                <p class="ppws-note"><?php esc_attr_e($args['description']) ?></p>
            <?php
            }
        }

        public function ppws_product_logged_in_user_role_settings($args)
        {
            global $ppws_product_options;
            $value = "";
            $blank_arr = array();
            $value = isset($ppws_product_options[$args['label_for']]) ? $ppws_product_options[$args['label_for']] : $blank_arr;
            if (isset($ppws_product_options[$args['label_for']]) && !empty($ppws_product_options[$args['label_for']]))            $value = explode(",", $ppws_product_options[$args['label_for']]);
                ...
                <div class="ppws_all_user_list">
                    ...
                    foreach ($editable_roles as $role) {
                        ...
                        <div class="ppws_all_user_list_input">
                            <label>
                                <input type="checkbox" class="ppws-checkbox ppws_editable_roles_single" name="ppws_product_settings[<?php esc_attr_e($args['label_for']) ?>][]" value="<?php esc_attr_e($role['name']); ?>" <?php if (is_array($value)) { if (in_array($role['name'], $value)) { esc_attr_e('checked'); } } ?>>
        ...
        public function ppws_product_select_user_role_settings($args)
        {
            global $ppws_product_options;
            $user_role = [ 'non-logged-in-user' ];
            if(isset($ppws_product_options[$args['label_for']])) {
                if(is_array($ppws_product_options[$args['label_for']])) {
                    $user_role = $ppws_product_options[$args['label_for']];
                }else{
                    $user_role = array($ppws_product_options[$args['label_for']]);
                }
            }
            if ($args['type'] == 'radio') {
            ?>
                <label class="ppws-label"><input type="checkbox" class="<?php esc_attr_e('ppws_user_non_logged_in_user'); ?>" name="ppws_product_settings[<?php esc_attr_e($args['label_for']); ?>][]" value="non-logged-in-user" <?php if(in_array('non-logged-in-user', $user_role)) echo 'checked="checked"'; ?>><?php _e('Non Logged In User', 'password-protected-store-for-woocommerce') ?></label>

                <label class="ppws-label"> <input type="checkbox" class="<?php esc_attr_e('ppws_user_logged_in_user'); ?>" name="ppws_product_settings[<?php esc_attr_e($args['label_for']); ?>][]" value="logged-in-user" <?php if(in_array('logged-in-user', $user_role)) echo 'checked="checked"'; ?>><?php _e('Logged In User', 'password-protected-store-for-woocommerce') ?></label>
        ...
        public function ppws_product_create_list_settings($args)
        {
            global $ppws_product_options;
            $value = isset($ppws_product_options[$args['label_for']]) ? explode(",",$ppws_product_options[$args['label_for']]) : array();

            if ($args['type'] == 'select') { ?>
                <select name="ppws_product_settings[<?php esc_attr_e($args['label_for']) ?>][]" class="ppws_product_products" id="ppws_product_products"  multiple="multiple">
                    <?php if(!empty($value)) {
                        foreach($value as $product_id) {
                            if(empty($product_id)) continue;
                            $product_title = get_the_title( $product_id ); ?>
                            <option selected="selected" value="<?php esc_attr_e($product_id); ?>"><?php echo $product_title." (#".$product_id.")"; ?></option>
                        <?php }
                    } ?>
                </select>
        ...
        public function product_sanitize_settings($input)
        {
            $new_input = array();
            ...
            if (isset($input['enable_user_role'])) {
                ...
                if (isset($input['product_select_user_role_field_radio'])) {
                    $new_input['product_select_user_role_field_radio'] = array_map( 'sanitize_text_field', $input['product_select_user_role_field_radio'] );
                }
