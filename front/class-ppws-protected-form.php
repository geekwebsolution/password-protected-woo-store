...
if (isset($_POST['ppws_submit'])) {
    ...
    do {
        if (isset($ppws_page_options['ppws_page_enable_password_field_checkbox']) == 'on' || isset($ppws_product_options['ppws_product_enable_password_field_checkbox']) == 'on' || isset($ppws_product_categories_options['ppws_product_categories_enable_password_field_checkbox']) == 'on') {
            if(is_page() || is_shop() ) {
                    ...
                    if(isset($ppws_product_options['ppws_product_enable_password_field_checkbox']) && $ppws_product_options['ppws_product_enable_password_field_checkbox'] == 'on') {
                        ...
                        if(isset($selected_products) && !empty($selected_products)) {
                            ...
                            if(in_array($product_id,$selected_products)) {
                                $ppws_main_password = $ppws_product_options['ppws_product_set_password_field_textbox'];
                                $ppws_set_password_expiry = $ppws_product_options['ppws_product_set_password_expiry_field_textbox'];
                                $ppws_expiry = (!empty($ppws_set_password_expiry)) ? ($ppws_set_password_expiry * 60 * 60 * 24) : (10 * 365 * 24 * 60 * 60);
                                if (ppws_decrypted_password($ppws_main_password) == $ppws_current_pass) {
    ...
    <div class="ppws_modal">
        ...
        <div class="ppws_modal_box">
            ...
            <div class="ppws_modal_box_text">
                <?php
                $logo_image = $logo_link_type = $logo_link_text = $logo_link_target = $logo_link_nofollow = "";   
                if(isset($ppws_style_option['form_page_logo_image_selecter'])) $logo_image = $ppws_style_option['form_page_logo_image_selecter'];
                if(isset($ppws_style_option['form_logo_link_type_field']))     $logo_link_type = $ppws_style_option['form_logo_link_type_field'];
                if(isset($ppws_style_option['form_logo_link_field_textbox']))  $logo_link_text = $ppws_style_option['form_logo_link_field_textbox'];
                if(isset($ppws_style_option['form_enable_logo_link_target_checkbox']))  $logo_link_target = $ppws_style_option['form_enable_logo_link_target_checkbox'];
                if(isset($ppws_style_option['form_enable_logo_link_nofollow_checkbox']))  $logo_link_nofollow = $ppws_style_option['form_enable_logo_link_nofollow_checkbox'];
                if(!empty($logo_image)) {
